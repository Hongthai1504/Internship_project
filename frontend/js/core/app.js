...
function toggleLanguage() {
  ...
  if (productListEl) {
    productListEl.innerHTML =
      "<p style='text-align:center;'>Đang tải lại dữ liệu / Loading data...</p>";
    allProducts = [];
    fetchProducts(1);
  }
}
...
async function fetchProducts(page = 1, isAppending = false) {
  try {
    ...
    const isHomePage =
      ...
      !urlParams.get("id") &&
      !urlParams.get("deals") &&
      !document.getElementById("wishlist-page-container");

    let fetchLimit = isHomePage ? 100 : 12;

    const response = await fetch(
      `${API_URL}?page=${page}&limit=${fetchLimit}&lang=${currentLang}`,
    );
    ...
    if (searchQuery) {
      ...
      if (listEl)
        listEl.innerHTML = `<div style="grid-column: 1/-1; text-align: center; padding: 50px;"><div style="font-size: 3rem; animation: spin 1s linear infinite;">⚙️</div><p style="color: #64748b; font-weight: bold; margin-top: 15px;">AI Semantic Engine is running...</p></div>`;
      ...
      try {
        ...
        if (aiRes.ok && aiData.ids && aiData.ids.length > 0) {
          currentPageProducts = allProducts.filter((p) =>
            aiData.ids.includes(p.id),
          );
        } else {
          currentPageProducts = [];
        }
    ...
    // 3. XỬ LÝ TRANG KHUYẾN MÃI (DEALS) - MỚI THÊM VÀO
      ...
      if (titleEl) {
        ...
        titleEl.innerHTML =
          currentLang === "vi" ? "⚡ Khuyến mãi cực HOT" : "⚡ Top Hot Deals";
        document.title =
          currentLang === "vi"
            ? "Khuyến mãi | Best Tech"
            : "Hot Deals | Best Tech";
      }
      currentPageProducts = allProducts
        .filter((p) => parseFloat(p.price) <= 1000)
    ...
    if (listEl) {
      listEl.innerHTML =
        "<p style='grid-column: 1/-1; text-align: center; color: red;'>Connection error. Please check your Backend server.</p>";
    }
  }
}
...
function renderDynamicBrands(products) {
  ...
  if (uniqueBrands.length === 0) {
    brandFiltersEl.innerHTML =
      "<li style='color: #64748b; font-size: 0.9rem; font-style: italic;'>No brands available</li>";
    return;
  }

  brandFiltersEl.innerHTML = uniqueBrands
    .map(
      (brand) => `
...
function handleFilters() {
  const activeBrandCheckboxes = document.querySelectorAll(
    ".dynamic-brand-checkbox:checked",
  );
  const selectedBrands = Array.from(activeBrandCheckboxes).map((cb) =>
  ...
  const isWishlist =
    document.getElementById("wishlist-page-container") !== null;
  let baseData = isWishlist
    ? window.currentWishlistData || []
    : currentPageProducts;
...
staticPriceRadios.forEach((radio) =>
...
function renderProducts(productsToDisplay) {
  ...
  if (productsToDisplay.length === 0) {
    productListEl.innerHTML =
      "<p style='grid-column: 1/-1; text-align: center; font-size: 1.2rem;'>We couldn't find any matches.</p>";
    return;
  }
...
function renderGroupedProducts(productsToDisplay) {
  ...
  if (productsToDisplay.length === 0) {
    productListEl.innerHTML =
      "<p style='text-align: center; font-size: 1.2rem; margin-top: 50px;'>We couldn't find any products matching your filter.</p>";
    return;
  }

  const categoryMap = {};
  globalCategories.forEach((c) => {
    categoryMap[c.id] = currentLang === "vi" ? c.name_vi || c.name : c.name;
  });
...
function showProductDetail(product_id) {
  ...
  if (document.getElementById("detail-sku"))
...
const openOrderHistoryModal = async () => {
  ...
  const loadingText =
    currentLang === "vi" ? "Đang tải dữ liệu..." : "Loading your orders...";
  if (historyList)
    historyList.innerHTML = `<div style="text-align: center; padding: 50px;"><div style="font-size: 2rem; animation: spin 1s linear infinite;">⚙️</div><p style="color: #64748b; margin-top: 10px; font-weight: bold;">${loadingText}</p></div>`;

  const token = localStorage.getItem("token");
  if (!token) {
    alert(
      currentLang === "vi" ? "Vui lòng đăng nhập lại!" : "Please log in again!",
    );
    window.location.href = "/pages/user/login.html";
    return;
  }
  ...
  try {
    ...
    if (profileRes.ok) {
      ...
      if (avatarEl)
        avatarEl.innerText = profile.full_name.charAt(0).toUpperCase();
    }
    ...
    const errText =
      currentLang === "vi" ? "Lỗi kết nối máy chủ:" : "Connection error:";
    if (historyList)
      historyList.innerHTML = `<p style='color: #ef4444; text-align: center; font-weight: bold;'>${errText} ${error.message}</p>`;
  }
};

if (historyTrigger)
  historyTrigger.addEventListener("click", openOrderHistoryModal);
...
function renderOrdersByStatus(statusFilter) {
  ...
  if (statusFilter !== "all") {
    filteredOrders = userOrderHistoryCache.filter(
  ...
  const getStatusStyle = (status) => {
    switch (status.toLowerCase()) {
      ...
      case "cancelled":
        return { color: "#e11d48", bg: "#fee2e2" };
      default:
        return { color: "#475569", bg: "#f1f5f9" };
    }
  };

  historyList.innerHTML = filteredOrders
    .map((order) => {
...
if (searchInput && searchSuggestions) {
  searchInput.addEventListener("input", function () {
    ...
    const thinkingText =
      currentLang === "vi" ? "✨ AI đang suy nghĩ..." : "✨ AI is thinking...";
      ...
      try {
        ...
        if (aiRes.ok && aiData.ids && aiData.ids.length > 0) {
          filtered = allProducts
            .filter((p) => aiData.ids.includes(p.id))
            .slice(0, 5);
        } else {
          filtered = allProducts
            .filter((product) => isProductMatch(product, searchTerm))
            .slice(0, 5);
        }
        ...
        const errorText =
          currentLang === "vi" ? "Hệ thống AI đang bận." : "AI System busy.";
...
async function fetchProductReviews(productId) {
  ...
  const loadingText =
    currentLang === "vi" ? "Đang tải đánh giá..." : "Loading reviews...";
  const noReviewsText =
    currentLang === "vi"
    ...
    listEl.innerHTML = reviews
      .map((r) => {
        ...
        const imageHtml = r.image_url
          ? `<img src="${r.image_url}" style="max-width: 120px; max-height: 120px; border-radius: 8px; margin-top: 15px; border: 1px solid #e0e6ef; object-fit: cover; cursor: pointer;">`
          : "";
...
if (reviewForm) {
  reviewForm.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/api/products/${currentDetailProductId}/reviews`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  chatForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        appendMessage(
          "bot",
          "Sorry, my system is currently experiencing issues.",
        );
      }
    } catch (error) {
      document.getElementById(typingId).remove();
  ...
  if (index > -1) {
    ...
    if (compareList.length > 0) {
      ...
      if (
        firstProduct &&
        productToAdd &&
        firstProduct.category_id !== productToAdd.category_id
      ) {
    ...
    if (compareList.length >= MAX_COMPARE) {
      ...
      const msg =
        lang === "vi"
          ...
          : "You can only compare up to 3 products at a time.";
      alert(msg);
      checkboxElem.checked = false;
      return;
    }
...
function renderCompareDock() {
  ...
  if (compareDockItems) {
    compareDockItems.innerHTML = compareList
      .map((id) => {
...
async function renderWishlistPage() {
  ...
  try {
    const lang = localStorage.getItem("besttech_lang") || "en";
    const res = await fetch(
      `http://localhost:3000/api/products?limit=1000&lang=${lang}`,
    );
    const result = await res.json();
    const allProductsData = result.data || [];

    const productsToDisplay = allProductsData.filter((p) =>
    ...
    container.innerHTML =
      '<p style="text-align: center; color: #ef4444; grid-column: 1/-1;">Connection error. Could not load wishlist items.</p>';
  }
}
...
function drawWishlistCards(products) {
  ...
  if (products.length === 0) {
    container.innerHTML =
      "<p style='grid-column: 1/-1; text-align: center; font-size: 1.1rem; color: #666; margin-top: 20px;'>Không tìm thấy sản phẩm nào phù hợp với bộ lọc.</p>";
    return;
  }
...
async function toggleMap(orderId, address) {
  ...
  try {
    ...
    alert(t.err_server);
    btn.innerHTML = t.btn_view_map;
>>>>>>> 5dcd539 (refactor: Tái cấu trúc logic Category và Shipper, tối ưu file app.js)
  }
}
...
const urlParams = new URLSearchParams(window.location.search);
if (!urlParams.has("id") && !urlParams.has("q") && !urlParams.has("deals")) {
  fetchProducts();
}
