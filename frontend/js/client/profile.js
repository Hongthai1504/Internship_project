...
if (tabInfo && tabAddr) {
  ...
  tabAddr.addEventListener("click", () => {
    ...
    // Tăng thời gian chờ lên 300ms để fix lỗi xám bản đồ
    setTimeout(() => {
      if (!map) {
        initMap();
      } else {
...
async function loadProfile() {
  try {
    ...
    if (res.ok) {
      ...
      document.getElementById("prof-avatar").innerText = data.full_name
        .charAt(0)
        .toUpperCase();
  ...
  profileUpdateForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        document.getElementById("prof-avatar").innerText = full_name
          .charAt(0)
          .toUpperCase();
      } else {
      ...
      submitBtn.innerText =
        profileLang === "vi" ? "Lưu thay đổi" : "Save Changes";
    }
  });
}
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (addresses.length === 0) {
        const emptyText =
          profileLang === "vi"
            ? "Bạn chưa có địa chỉ nào. Hãy thêm ở bên dưới!"
      ...
      const btnSetDefault =
        profileLang === "vi" ? "Đặt Mặc định" : "Set Default";
...
async function setDefaultAddress(id) {
  try {
    ...
    console.error("Error setting default");
  }
}
...
async function reverseGeocode(lat, lng) {
  ...
  finalInput.value =
    profileLang === "vi" ? "Đang xác thực..." : "Validating...";

  try {
    const res = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=18&addressdetails=1&email=htzon1504@gmail.com`,
    );
    ...
    if (data && data.display_name) {
      ...
      alert(
        profileLang === "vi"
          ? "Không thể xác định địa chỉ tại vị trí này."
    ...
    finalInput.value = "";
    alert(
      profileLang === "vi" ? "Lỗi kết nối bản đồ." : "Map connection error.",
    );
  }
}
...
if (btnFindMap) {
  btnFindMap.addEventListener("click", async () => {
    ...
    if (!query) {
      return alert(
        profileLang === "vi"
          ? "Vui lòng nhập địa chỉ cần tìm!"
          : "Please enter an address to search!",
      );
    }

    const originalText = btnFindMap.innerHTML;
    ...
    try {
      ...
      if (data && data.features && data.features.length > 0) {
        ...
        // Lưu ý: Photon trả về mảng tọa độ theo thứ tự [Kinh độ, Vĩ độ]
        ...
        document.getElementById("final-validated-address").value = displayName;
      } else {
        alert(
          profileLang === "vi"
            ? "Không tìm thấy địa chỉ! Hãy thử gõ Phường/Quận."
            : "Address not found!",
        );
      ...
      alert(
        profileLang === "vi"
          ? "Lỗi tìm kiếm do đường truyền."
          : "Search error.",
      );
    } finally {
      btnFindMap.innerHTML = originalText;
    }
  });
}
...
if (btnSaveAddress) {
  btnSaveAddress.addEventListener("click", async () => {
    const profileLang = localStorage.getItem("besttech_lang") || "en";
    const finalAddress = document.getElementById(
      "final-validated-address",
    ).value;

    if (
      !finalAddress ||
      finalAddress === "Đang xác thực..." ||
      finalAddress === "Validating..."
    ) {
      return alert(
        profileLang === "vi"
          ? "Vui lòng sử dụng bản đồ để ghim một địa chỉ hợp lệ!"
          : "Please use the map to pin a valid address!",
      );
    }
